...
use crate::*;
use near_sdk::{near_bindgen, utils::assert_one_yocto, AccountId, Promise};
...
impl OwnerInterface for SlotMachine {
    ...
    fn get_contract_state(&self) -> std::collections::HashMap<String, String> {
        ...
        state.insert(
            String::from("bet_payment_adjustment"),
            self.bet_payment_adjustment.to_string(),
        );
        ...
        state.insert(
            String::from("owner_balance"),
            self.owner_balance.to_string(),
        );
        state.insert(
            String::from("house_balance"),
            self.house_balance.to_string(),
        );
        state.insert(String::from("max_bet"), self.max_bet.to_string());
        state.insert(String::from("min_bet"), self.min_bet.to_string());
        state.insert(
            String::from("min_balance_fraction"),
            self.min_balance_fraction.to_string(),
        );
    ...
    fn create_new_partner(
        ...
        assert_one_yocto();
        assert!(
            !self.game_structs.contains_key(&nft_contract),
            "Collection is already initialized"
        );
        ...
        self.game_balances.insert(
            &nft_contract,
            &LookupMap::new(format!("{}{}", nft_contract, "game_struct".to_string()).into_bytes()),
        );
    ...
    fn alter_partner(
        ...
        assert!(
            self.game_structs.contains_key(&nft_contract),
            "Collection isn't yet initialized"
        );
        let game_settings = PartneredGame {
            partner_owner,
            blocked,
...
mod tests {
    ...
    use super::*;
    use crate::tests::{get_context, sample_contract, OWNER_ACCOUNT, SIGNER_ACCOUNT};
    ...
    fn test_panic_button() {
        ...
        assert!(
            !contract.panic_button,
            "panic button is being initialized as true"
        );
        ...
        assert!(
            !contract.panic_button,
            "panic button didn't change back to false"
        );
    }
    ...
    fn test_get_contract_state() {
        ...
        assert_eq!(
            &contract.owner_id.to_string(),
            view_call.get("owner_id").unwrap()
        );
        assert_eq!(
            &contract.nft_account.to_string(),
            view_call.get("nft_account").unwrap()
        );
        assert_eq!(
            &contract.panic_button.to_string(),
            view_call.get("panic_button").unwrap()
        );
        assert_eq!(
            &contract.bet_payment_adjustment.to_string(),
            view_call.get("bet_payment_adjustment").unwrap()
        );
        assert_eq!(
            &contract.nft_fee.to_string(),
            view_call.get("nft_fee").unwrap()
        );
        assert_eq!(
            &contract.owner_fee.to_string(),
            view_call.get("owner_fee").unwrap()
        );
        assert_eq!(
            &contract.house_fee.to_string(),
            view_call.get("house_fee").unwrap()
        );
        assert_eq!(
            &contract.nft_balance.to_string(),
            view_call.get("nft_balance").unwrap()
        );
        assert_eq!(
            &contract.owner_balance.to_string(),
            view_call.get("owner_balance").unwrap()
        );
        assert_eq!(
            &contract.house_balance.to_string(),
            view_call.get("house_balance").unwrap()
        );
        assert_eq!(
            &contract.max_bet.to_string(),
            view_call.get("max_bet").unwrap()
        );
        assert_eq!(
            &contract.min_bet.to_string(),
            view_call.get("min_bet").unwrap()
        );
        assert_eq!(
            &contract.min_balance_fraction.to_string(),
            view_call.get("min_balance_fraction").unwrap()
        );
        assert_eq!(
            &contract.max_odds.to_string(),
            view_call.get("max_odds").unwrap()
        );
        assert_eq!(
            &contract.min_odds.to_string(),
            view_call.get("min_odds").unwrap()
        );
    }
    ...
    fn test_update_contract() {
        ...
        assert_ne!(
            contract.bet_payment_adjustment,
            new_bet_payment_adjustment.0
        );
        ...
        contract.update_contract(
            new_bet_payment_adjustment,
            new_nft_fee,
            new_owner_fee,
            new_house_fee,
            new_max_bet,
            new_min_bet,
            new_min_balance_fraction,
            new_max_odds,
            new_min_odds,
        );
