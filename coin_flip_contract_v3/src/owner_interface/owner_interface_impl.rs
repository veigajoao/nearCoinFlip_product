...
impl OwnerInterface for SlotMachine {
    ...
    fn get_contract_state(&self) -> std::collections::HashMap<String, String> {
        ...
        //these must be moved into the partnered game
    ...
    fn retrieve_owner_funds(&mut self) -> Promise {
        ...
        let token_address = self.owner_balance.keys().next().expect("No balances to withdraw");
        let token_balance = self.owner_balance.remove(&token_address).unwrap();

        Promise::new(token_address)
            .function_call(
                b"ft_transfer".to_vec(),
                json!({
                    "receiver_id": self.owner_id,
                    "amount": token_balance.to_string(),
                    "memo": "coin flip owner withdraw"
    ...
    fn retrieve_nft_funds(&mut self) -> Promise {
        ...
        let token_address = self.nft_balance.keys().next().expect("No balances to withdraw");
        let token_balance = self.nft_balance.remove(&token_address).unwrap();

        Promise::new(token_address)
            .function_call(
                b"ft_transfer".to_vec(),
                json!({
                    "receiver_id": self.nft_account,
                    "amount": token_balance.to_string(),
                    "memo": "coin flip owner withdraw"
                }).to_string().as_bytes().to_vec(),
                1,
    ...
    fn create_new_partner(&mut self, partner_owner: AccountId, nft_contract: AccountId, token_contract: AccountId, partner_fee: U128) -> bool {
        ...
        let game_settings = PartneredGame {
            partner_owner,
            blocked: false,
            token: TokenType::FT(token_contract),
    ...
    fn alter_partner(&mut self, partner_owner: AccountId, nft_contract: AccountId, partner_fee: U128, 
        ...
        let previous_game_settings = self.game_structs.get(&nft_contract).unwrap();
        let game_settings = PartneredGame {
            partner_owner,
            blocked,
            token: previous_game_settings.token,
            partner_fee: partner_fee.0, // base 10e-5
            partner_balance: 0,
            sub_house_balance: previous_game_settings.sub_house_balance,
        };
        self.game_structs.insert(&nft_contract, &game_settings);
        true
    }
