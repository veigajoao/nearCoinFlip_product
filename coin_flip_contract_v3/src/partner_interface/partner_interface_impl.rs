...
impl PartnerInterface for SlotMachine {
    ...
    fn view_partner_data(&self, nft_contract: AccountId) -> HashMap<String, String> {
        ...
        match game_struct.token {
            FT(i) => state.insert(String::from("token"), i.to_string())
        };
        ...
        state.insert(String::from("sub_house_balance"), game_struct.sub_house_balance.to_string());
    ...
    fn retrieve_partner_balance(&mut self, nft_contract: AccountId) -> Promise {
        ...
        self.game_structs.insert(&nft_contract, &game_struct);
            ...
            FT(i) => (
                Promise::new(i)
                    .function_call(
                        ...
                        json!({
                            "receiver_id": game_struct.partner_owner.clone(),
                            "amount": balance.to_string(),
                            "memo": "coin flip partner withdraw"
    ...
    fn retrieve_sub_house_funds(&mut self, nft_contract: AccountId, amount: u128) -> Promise {
        let mut game_struct = self.game_structs.get(&nft_contract).expect("No partner found for this contract").clone();
        assert!(game_struct.partner_owner == env::predecessor_account_id(), "Only owner of partnered game can call this function");
        assert_one_yocto();

        game_struct.withdraw_house_funds(amount);
        self.game_structs.insert(&nft_contract, &game_struct);
...
impl PartneredGame {
    ...
    pub fn deposit_sub_house_funds(&mut self, amount: u128) {

        self.sub_house_balance += amount;

    }

    pub fn withdraw_house_funds(&mut self, amount: u128) {

        assert!(self.sub_house_balance >= amount, "insufficient sub_house_balance in PartneredGame");
        self.sub_house_balance -= amount;
